...
async function updateViewportsForSegmentationRendering({
  ...
  const { cornerstoneViewportService, segmentationService, viewportGridService, toolGroupService } =
  ...
  // leave only active segmentation, turn off other segmentations (only visibility)
  //segmentationService.segmentations = Object.fromEntries(Object.entries(segmentationService.segmentations).filter(([k,v]) => v.isActive==false).forEach(([k,v]) => v.isVisible==false));
  Object.values(segmentationService.segmentations).forEach(obj => {
    if ((obj.isActive === false) && (obj.isVisible=== true)) {
      segmentationService.toggleSegmentationVisibility(obj.displaySetInstanceUID)
    }
    //if (obj.isActive === true && !toolGroupService.toolGroupIds.has('SEGToolGroup-default')) {
    //  segmentationService.jumpToSegmentCenter(obj.displaySetInstanceUID, 1, "default")
    //}
  });
  ...
  updatedViewports.forEach(async viewport => {
    ...
    //segmentationService.jumpToSegmentCenter(displaySet.displaySetInstanceUID, 1, 'default')
