...
class BasicInferTask(InferTask):
    ...
    def __call__(
        ...
        if "pos_points" in data:
            ...
            # Find the directory containing DICOM files
            # The NIfTI file is in the same directory as the DICOM files
            dicom_dir = os.path.dirname(data['image'])
            
            # Try multiple patterns to find DICOM files
            image_files = []
            for pattern in ['*.dcm', '*.512.dcm', '*']:
                potential_files = glob(os.path.join(dicom_dir, pattern))
                # Filter to only include files (not directories) and exclude non-DICOM files
                image_files = [f for f in potential_files if os.path.isfile(f) and 
                              not f.endswith('.nii.gz') and not f.endswith('.lock') and 
                              not f.endswith('.json') and not f.endswith('.txt')]
                if image_files:
                    break
            ...
            dcm_img_sample = dcmread(image_files[0], stop_before_pixels=True)
                ...
                class DefaultDicomSample:
                    def keys(self):
                        return []
                dcm_img_sample = DefaultDicomSample()
            ...
            for i in range(len(ann_frame_list)):
                ...
                # Fallback to image_files if GDCM doesn't find series
                if not dicom_filenames and image_files:
                ...
                dcm_img_sample_2 = dcmread(dicom_filenames[1], stop_before_pixels=True)
                    ...
                    logger.warning(f"No DICOM files found for frame ordering, using default frame index")
                    dcm_img_sample = dcm_img_sample_2 = None
                
                instanceNumber = None
                instanceNumber2 = None

                if dcm_img_sample and 0x00200013 in dcm_img_sample.keys():
                    instanceNumber = dcm_img_sample[0x00200013].value
                logger.info(f"Prompt First InstanceNumber: {instanceNumber}")
                if dcm_img_sample_2 and 0x00200013 in dcm_img_sample_2.keys():
